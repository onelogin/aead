...
class AEAD::Cipher::AES_256_CTR_HMAC_SHA_256 < AEAD::Cipher
  ...
  def _encrypt(nonce, plaintext, aad)
    self.cipher(:encrypt) do |cipher|
      ...
      mac        = hmac_generate(self.key, nonce, ciphertext, aad)
  ...
  def _decrypt(nonce, ciphertext, aad, tag)
    raise SecurityError, 'ciphertext failed authentication step' unless
      hmac_verify(self.key, nonce, ciphertext, aad, tag)
  ...
  def hmac_generate(key, nonce, ciphertext, aad)
    OpenSSL::HMAC.digest('SHA256', key, ciphertext + nonce + aad.to_s)
  end

  def hmac_verify(key, nonce, ciphertext, aad, hmac)
    self.class.signature_compare(
      hmac, hmac_generate(key, nonce, ciphertext, aad)
    )
  end
end
